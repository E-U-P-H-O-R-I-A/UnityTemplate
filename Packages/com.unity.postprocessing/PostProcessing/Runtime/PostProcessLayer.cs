...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    public sealed class PostProcessLayer : MonoBehaviour
    {
        ...
        void OnRenderImage(RenderTexture src, RenderTexture dst)
        {
            ...
            if (!finalBlitToCameraTarget && m_CurrentContext.IsUpscalingActive())
            {
...
#if UNITY_EDITOR
            // Automatically fix broken PostProcessResources references
            if (m_Resources == null)
            {
                string[] assetGuids = UnityEditor.AssetDatabase.FindAssets($"t:PostProcessResources");
                if (assetGuids != null && assetGuids.Length > 0)
                {
                    string assetPath = UnityEditor.AssetDatabase.GUIDToAssetPath(assetGuids[0]);
                    m_Resources = UnityEditor.AssetDatabase.LoadAssetAtPath<PostProcessResources>(assetPath);
                }
            }
#endif
        ...
        void LateUpdate()
        {
            ...
            if (m_Camera.targetTexture != null && m_CurrentContext.IsUpscalingActive())
            {
        ...
        void OnPreCull()
        {
            ...
            if (m_CurrentContext.IsTemporalAntialiasingActive() || m_CurrentContext.IsUpscalingActive())
            {
            ...
            // TND FIX: do not render the right eye twice
            if (m_Camera.stereoActiveEye == Camera.MonoOrStereoscopicEye.Right)
                return;
        ...
        void BuildCommandBuffers()
        {
            ...
            {
                // Ensure all upscaling resources are released when it's not in use
                upscaling.Release();
            ...
            if (context.stereoActive)
                context.UpdateSinglePassStereoState(context.IsTemporalAntialiasingActive() || context.IsUpscalingActive(), aoSupported, isScreenSpaceReflectionsActive);
#endif
            ...
            if (!finalBlitToCameraTarget && m_CurrentContext.IsUpscalingActive())
            {
                var displaySize = upscaling.DisplaySize;
        ...
        public void Render(PostProcessRenderContext context)
        {
            ...
            for (int eye = 0; eye < context.numberOfEyes; eye++)
            {
                ...
                else if (context.IsUpscalingActive())
                {
                    ...
                    context.SetRenderSize(upscaling.DisplaySize);
