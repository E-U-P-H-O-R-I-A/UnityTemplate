...
{
    public class SchemePropertyInspector
    {
        private const float SECTION_HEADER_HEIGHT = 20f;
        private const float SMALL_BUTTON_WIDTH = 22f;
        private const string SCHEMES_PROPERTY = "schemes";
        private const string SINGLE_PROPERTY = "scheme";
        private const string ID_PROPERTY = "id";
        private const string NEW_SCHEME_ID = "NewScheme";
        ...
        public bool Draw(SerializedObject serializedObject, DataEditorSkin currentSkin, float labelWidth)
        {
            ...
            var schemes = serializedObject.FindProperty(SCHEMES_PROPERTY);
            var single = serializedObject.FindProperty(SINGLE_PROPERTY);

            if (schemes == null && single == null)
            {
                DataEditorGUI.DrawEmptyState(skin, "This model has no schemes");
                return false;
            }
            ...
            if (schemes != null && schemes.isArray)
                DrawCollection(serializedObject, schemes);
            else
                DrawSingle(single);
        ...
        private void DrawCollection(SerializedObject serializedObject, SerializedProperty schemes)
        {
            DrawAddSchemeRow(serializedObject);

            if (schemes.arraySize == 0)
                DataEditorGUI.DrawEmptyState(skin, "No schemes yet, add the first one");

            for (int i = 0; i < schemes.arraySize; i++)
                DrawScheme(serializedObject, schemes, i);
        }

        private void DrawSingle(SerializedProperty scheme)
        {
            using (new EditorGUILayout.VerticalScope(skin.Section))
            using (new EditorGUILayout.VerticalScope(skin.SectionContent))
                DrawSchemeFields(scheme);
        }

        private void DrawAddSchemeRow(SerializedObject serializedObject)
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                GUILayout.FlexibleSpace();

                if (GUILayout.Button("Add Scheme", skin.SmallButton))
                    pending = () => AddScheme(serializedObject);
            }

            GUILayout.Space(2f);
        }

        private void DrawScheme(SerializedObject serializedObject, SerializedProperty schemes, int index)
        {
            var element = schemes.GetArrayElementAtIndex(index);
            ...
            using (new EditorGUILayout.VerticalScope(skin.Section))
            {
                ...
                using (new EditorGUILayout.VerticalScope(skin.SectionContent))
                {
                    ...
                    DrawSchemeFields(element);
                }
            }
        }

        private void DrawSchemeFields(SerializedProperty scheme)
        {
            var iterator = scheme.Copy();
            var end = scheme.GetEndProperty();
        ...
        private bool DrawSectionHeader(string label, string key, SerializedObject serializedObject, int index)
        {
            ...
            if (GUI.Button(buttonRect, skin.RemoveIcon, skin.SmallButton))
                pending = () => RemoveScheme(serializedObject, index);

            return isExpanded;
        }

        private static void AddScheme(SerializedObject serializedObject)
        {
            var schemes = serializedObject.FindProperty(SCHEMES_PROPERTY);
            if (schemes == null)
                return;
            ...
            var added = schemes.GetArrayElementAtIndex(schemes.arraySize - 1);
            var idProperty = added.FindPropertyRelative(ID_PROPERTY);

            if (idProperty != null)
                idProperty.stringValue = GetFreeSchemeId(schemes);

            serializedObject.ApplyModifiedProperties();
        }

        private static void RemoveScheme(SerializedObject serializedObject, int index)
        {
            var schemes = serializedObject.FindProperty(SCHEMES_PROPERTY);

            if (schemes == null || index < 0 || index >= schemes.arraySize)
                return;

            schemes.DeleteArrayElementAtIndex(index);
            serializedObject.ApplyModifiedProperties();
        }

        private static string GetFreeSchemeId(SerializedProperty schemes)
        {
            var taken = new HashSet<string>();

            for (int i = 0; i < schemes.arraySize; i++)
            {
                var idProperty = schemes.GetArrayElementAtIndex(i).FindPropertyRelative(ID_PROPERTY);

                if (idProperty != null)
                    taken.Add(idProperty.stringValue);
            }

            if (!taken.Contains(NEW_SCHEME_ID))
                return NEW_SCHEME_ID;

            for (int index = 1; index < int.MaxValue; index++)
            {
                string candidate = $"{NEW_SCHEME_ID}{index}";
