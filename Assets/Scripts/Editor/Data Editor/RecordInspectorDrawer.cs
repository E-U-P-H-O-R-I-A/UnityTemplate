...
namespace Editor.Data_Editor
{
    public class RecordInspectorDrawer
    {
        ...
        public bool Draw(object container, string ownerKey, DataEditorSkin currentSkin, float labelWidth)
        {
            skin = currentSkin;
            changed = false;
            rowIndex = 0;

            var records = RecordReflection.GetRecords(container).ToList();

            if (records.Count == 0)
            {
                DataEditorGUI.DrawEmptyState(skin, "This save has no serialized records");
                return false;
            }
            ...
            if (RecordReflection.IsCollection(container))
                DrawCollection(records, ownerKey);
            else
                DrawSingle(records[0], ownerKey);
        ...
        private void DrawCollection(List<IRecord> records, string ownerKey)
        {
            for (int i = 0; i < records.Count; i++)
            {
                ...
                {
                    if (!DrawSectionHeader(record.Id, key))
                        continue;

                    GUILayout.Space(2f);
                    DrawSerializableFields(record, key);
                }
            }
        }

        private void DrawSingle(IRecord record, string ownerKey)
        {
            using (new EditorGUILayout.VerticalScope(skin.Section))
            using (new EditorGUILayout.VerticalScope(skin.SectionContent))
                DrawSerializableFields(record, ownerKey);
        }
        ...
        {
            foreach (var field in RecordReflection.GetSerializableFields(target.GetType()))
            {
