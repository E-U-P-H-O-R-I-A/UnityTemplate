		...
		SubShader
		{
			...
			Pass
			{
				...
				half4 frag(v2f i) : SV_Target
				{
					...
					col.g += pow( saturate(dot(i.normal, -_XFurVFX1Direction.xyz)), _FXFalloff1) * _FXAdd1 * _XFurVFX1GlobalAdd * length(vfx1.rgb);
					col.g = clamp(col.g, 0, min(length(vfx1.rgb*1.25), 1));
					col.b += saturate(dot(i.normal, -_XFurVFX2Direction.xyz) - _FXFalloff2) * _FXAdd2 * _XFurVFX2GlobalAdd;
					col.b = clamp(col.b, 0, min(length(vfx2.rgb * 1.25), 1));
					col.a += saturate(dot(i.normal, -_XFurVFX1Direction.xyz) - _FXFalloff3) * _FXAdd3 * _XFurVFX3GlobalAdd;
					col.r -= _FXMelt0;
					col.g -= _FXMelt1;
					col.b -= _FXMelt2;
					col.a -= _FXMelt3;
