        ...
        void OnTriggerStay(Collider other) {
            ...
            if ( xfurSystem ) {
                if ( PainterObjectShape == PainterShape.Spherical ) {
                    ...
                    if (PaintDataMode == XFurStudioAPI.PaintDataMode.FurGrooming ) {
                        for ( int i = 0; i < xfurSystem.FurDataProfiles.Length; i++ ) {
                            if ( xfurSystem.MainRenderer.isFurMaterial[i] )
                                XFurStudioAPI.Groom( xfurSystem, i, transform.position, ( transform.position - xfurSystem.transform.position ).normalized, Radius, intensity, hardness, (transform.position-prevPos).normalized );
                        }
                        return;
                    }
                    ...
                    for ( int i = 0; i < xfurSystem.FurDataProfiles.Length; i++ ) {
                        if (xfurSystem.MainRenderer.isFurMaterial[i])
                            XFurStudioAPI.Paint( xfurSystem, PaintDataMode, i, transform.position, ( transform.position - xfurSystem.transform.position ).normalized, Radius, intensity, hardness, color, Texture2D.whiteTexture );
                    }
                }
            }
        }
